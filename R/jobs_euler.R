...
while (i <= n) {
  ...
  write("#PBS -l select=1:ncpus=10",
        file = output.file,
        append = TRUE)
  
  write("#PBS -l walltime=128:00:00",
        file = output.file,
        append = TRUE)
  
  write("#PBS -m abe", file = output.file, append = TRUE)
  
  write("#PBS -M elainegatto@estudante.ufscar.br",
        file = output.file,
        append = TRUE)
  
  write(" ", file = output.file, append = TRUE)
  
  write("eval \"$(conda shell.bash hook)\" ",
        file = output.file,
        append = TRUE)
  ...
  str7 = paste(
    "Rscript /mnt/nfs/home/elaine/CrossValidationMultiLabel/R/cvm.R ",
    dataset$Id,
    " 10 10 ",
    "\"/lustre/elaine/cv-",
    dataset$Name,
